...
class XConditionNetwork(nn.Module):
    ...
    def __init__(self, d_model, x_cond_dim=64, n_heads=4, dropout=0.1):
        """
        Args:
        ...
        # === Y state projection (project c_out to x_cond_dim) ===
        # Dynamically created in forward since c_out may be unknown at init time
        self.y_proj = None
    ...
    def forward(self, x_enc, y_state, c_out):
        """
        Args:
            x_enc: encoder output (B, T, d_model) - condition features
            y_state: current state of Y (B, O, c_out) - Y during denoising
            c_out: dimension of Y (used for dynamically creating projection layer)
        ...
        # x_local: (B, O, x_cond_dim)

        # === Y state projection ===
        # Dynamically create projection layer (if needed)
        if self.y_proj is None or self.y_proj.in_features != c_out:
            self.y_proj = nn.Linear(c_out, self.x_cond_dim).to(x_enc.device)
...
class ConditionalGuidedModel(nn.Module):
    def __init__(self, diff_steps, enc_in, c_out=None, d_model=32, x_cond_dim=64, n_heads=4, dropout=0.1):
        ...
        Args:
            ...
            enc_in: condition input dimension (X + Y_history = 43 dimensions)
    ...
    def forward(self, x, y_t, y_0_hat, g_x, t):
        ...
        x_cond = self.x_condition_net(x, y_state, self.c_out * 2)  # (B, O, x_cond_dim)
