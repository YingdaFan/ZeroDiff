...
class ProbForecastExp(ForecastExp):
    ...
    def _evaluate(self, dataloader):
        ...
        self.metrics.reset()
        results = []
        # CRITICAL: torch.no_grad() disables gradient computation, saving massive memory
        with torch.no_grad(), tqdm(total=len(dataloader.dataset)) as progress_bar:
            for batch_x, batch_y, origin_x, origin_y, batch_x_date_enc, batch_y_date_enc, *rest in dataloader:
                # *rest captures any extra elements (e.g., is_masked for RAW pipeline)
                ...
                batch_x_date_enc = batch_x_date_enc.to(self.device).float()
                batch_y_date_enc = batch_y_date_enc.to(self.device).float()
                # Use mixed precision for inference to reduce memory usage
